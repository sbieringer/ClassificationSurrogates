...
import sys
sys.path.append('..')
from models.custom_linear_flipout import custom_LinearFlipout

from normflows.utils.nn import tile
...
class MaskedLinear(custom_LinearFlipout):
    ...
    def forward(self, x):
        self.sample_weights()
        return F.linear(x, (self.mu_weight + self.delta_weight) * self.mask, self.mu_bias+self.bias)
...
class MaskedResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        if zero_initialization:
            init.uniform_(self.linear_layers[-1].mu_weight, a=-1e-3, b=1e-3)
            init.uniform_(self.linear_layers[-1].mu_bias, a=-1e-3, b=1e-3)
